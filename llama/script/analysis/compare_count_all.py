"""
Compare baseline H2O routing and count-refined H2O routing (all visible tokens).

Count refinement rule (all visible tokens):
    refined_logit = qk + log(C)
where C is the size of the belong-set mapped to that visible key.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare baseline H2O routing and count-refined H2O routing for all visible tokens."
    )
    add_common_compare_args(
        parser,
...
def build_count_refined_alpha_all(qk_logits, mask, belong, count, pos_list):
    ...
    for h in range(n_heads):
        for i, pos in enumerate(pos_list):
            ...
            if int(counts.sum().item()) != total_available:
                raise ValueError(
                    f"count should sum to total_available, got {int(counts.sum().item())} vs {total_available}"
                )
            ...
            if idx.numel() > 0:
                c = counts[idx]
                if int(c.min().item()) < 1:
                    raise ValueError("count should be >= 1 for visible tokens")
                logits[h, i, idx] = logits[h, i, idx] + torch.log(c.float())
...
def main():
    ...
    alpha_count_all = build_count_refined_alpha_all(
        qk_logits=qk_logits,
        mask=route_mask,
        belong=belong,
        pos_list=pos_list,
    ...
    count_all_metric = v_l2_per_pos(alpha_count_all.detach().to(torch.float32), v_head, v_gt)

    print("===== Compare-Count-All Summary =====")
    print(
    ...
    print(
        ...
        f"mean count_all v_l2={float(count_all_metric.mean().item()):.8e}, "
        f"mean improvement={float((base_metric - count_all_metric).mean().item()):.8e}"
    )
    ...
    save_per_pos_metric_tsv(
        ...
        other_metric=count_all_metric,
        other_name="count_all",
    )
    ...
    plot_per_pos_two_lines(
        ...
        y2=count_all_metric,
        label1="base_v_l2",
        label2="count_all_v_l2",
        title="Per-Position V-L2: Base vs Count-All-Refined",
        dpi=args.plot_dpi,
    )
    ...
    stats = {
        ...
        "mean_count_all_metric": float(count_all_metric.mean().item()),
        "mean_improvement": float((base_metric - count_all_metric).mean().item()),
        "base_metric_per_pos": base_metric.detach().cpu(),
        "count_all_metric_per_pos": count_all_metric.detach().cpu(),
    }
    stats_path = os.path.join(output_dir, "compare_count_all_stats.pt")
