"""
Compare baseline H2O routing and oracle-all refined routing.

For heavy-hitter representative j at each (head, pos):
- avgK_j = sumK_j / C_j
- avgV_j = sumV_j / C_j
- delta_k(x) = k_x - avgK_j, for x in cluster(j)
- coeff_v(x) = projection coefficient of V_x on avgV_j direction:
      coeff_v(x) = <V_x, avgV_j> / ||avgV_j||^2
- alpha_j = sum_x exp(q·delta_k(x)) * coeff_v(x)
Routing logits on HH representatives become:
      q·avgK_j + log(alpha_j)
Value vectors use avgV_j on HH representatives.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare baseline H2O routing and oracle-all refined routing."
    )
    add_common_compare_args(
        parser,
...
def build_oracle_all_refined_alpha(
    qk_logits,
    q_head,
    k_head,
    v_head,
    ...
    if k_head.shape[0] != n_heads or v_head.shape[0] != n_heads:
        raise ValueError(
            f"k/v head shape mismatch: k={tuple(k_head.shape)}, v={tuple(v_head.shape)}"
        )
    ...
    for h in range(n_heads):
        ...
        for i, pos in enumerate(pos_list):
            ...
            sumv = hh_sumv_val[h][i]
            if idx is None or sumk is None or sumv is None or idx.numel() == 0:
                continue
            ...
            for t, rep_tensor in enumerate(idx):
                ...
                score = (delta_k @ q) / scale

                denom = avgv.pow(2).sum().clamp_min(eps)
                coeff_v = (v_head[h, member_idx].float() @ avgv) / denom

                alpha = (torch.exp(score) * coeff_v).sum()
                alpha_safe = alpha.clamp_min(eps)

                q_avgk = (q * avgk).sum() / scale
                logits[h, i, rep] = q_avgk + torch.log(alpha_safe)
...
def main():
    ...
    alpha_oracle_all = build_oracle_all_refined_alpha(
        qk_logits=qk_logits,
        q_head=q_head,
        k_head=k_head,
        v_head=v_head,
    ...
    v_oracle_all = build_avgv_refined_v(
        alpha=alpha_oracle_all,
        v_head=v_head,
    ...
    oracle_all_metric = v_l2_per_pos_from_v(v_oracle_all, v_gt)

    print("===== Compare-Count-Oracle-All Summary =====")
    print(
    ...
    print(
        ...
        f"mean oracle_all v_l2={float(oracle_all_metric.mean().item()):.8e}, "
        f"mean improvement={float((base_metric - oracle_all_metric).mean().item()):.8e}"
    )
    ...
    save_per_pos_metric_tsv(
        ...
        other_metric=oracle_all_metric,
        other_name="oracle_all",
    )
    ...
    plot_per_pos_two_lines(
        ...
        pos_list=pos_list,
        y1=base_metric,
        y2=oracle_all_metric,
        label1="base_v_l2",
        label2="oracle_all_v_l2",
        title="Per-Position V-L2: Base vs Oracle-All with budget={:.2f}".format(args.budget),
        dpi=args.plot_dpi,
    )
    ...
    stats = {
        ...
        "mean_oracle_all_metric": float(oracle_all_metric.mean().item()),
        "mean_improvement": float((base_metric - oracle_all_metric).mean().item()),
        "base_metric_per_pos": base_metric.detach().cpu(),
        "oracle_all_metric_per_pos": oracle_all_metric.detach().cpu(),
    }
    stats_path = os.path.join(output_dir, "compare_count_oracle_all_stats.pt")
