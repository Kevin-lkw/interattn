...
9. **Lazy stats collection**
   - 默认继续记录 condition-block budget stats，保证 eps sweep 输出可直接分析等效 budget。
...
lazy stats 公式对齐:            完全一致
GovReport 4-token stats-on/off: token 完全一致
```
...
| Triton，保留 stats（旧同步统计） | 4.24 s/sample | 1.67× |
...
### Lazy stats 后的默认 stats-on 性能

后续将 stats 统计改为 lazy materialization 后，默认记录 stats 的性能明显改善。
设置：GovReport 前 5 条、128 tokens、`block_size=32`、`eps=0.1`。

| 模式 | 平均耗时 |
|---|---:|
| stats-on，旧同步统计 | 4.01 s/sample |
| stats-on，lazy stats | 3.51 s/sample |
| stats-off | 3.02 s/sample |

lazy stats 使默认 stats-on 路径提升约 12.5%。stats-off 仍然最快，但默认
stats-on 已不再有大量 per-layer `.item()` 同步气泡。
...
测试：GovReport 第一条，输入 10,733 tokens，生成 32 tokens，32 层；kernel 已预热，关闭 stats。下表为 3.20 s/sample 稳定版 profile 的结果。
...
同一条 GovReport、32 tokens、`block_size=32`、`eps=0.1`：

| 模式 | kernel span | busy sum | 估算 GPU busy | gap sum |
|---|---:|---:|---:|---:|
| stats-on，旧同步统计 | 1644 ms | 1143 ms | 69.6% | 500 ms |
| stats-on，lazy stats | 1370 ms | 1138 ms | 83.0% | 232 ms |
| stats-off | 1218 ms | 1105 ms | 90.7% | 113 ms |

lazy stats 基本消除了最坏的同步气泡，但 stats-on 仍会额外写 selected mask、
做 GPU reduce，并在最终 JSON metadata 汇总时同步一次，因此仍慢于 stats-off。
当前 stats-on 的主要 CUDA 时间仍是：

```text
model GEMM/GEMV:       ~710 ms
prefill FlashAttention: ~98 ms
KV cache cat:           ~64 ms
sparse attention:       ~37 ms
condition selection:    ~15 ms
```
...
默认记录 stats；这是 eps sweep 推荐模式，因为输出包含 `condition_block_equiv_budget`。
如果只做纯速度测试，可关闭 stats：
...
PYTHONPATH=llama \
conda run -n nanogpt python -m script.analysis.condition_block_gen.longbench.run \
  --device cuda:0 \
  --dataset gov_report \
...
```bash
# 默认 COLLECT_STATS=1，会记录 budget stats
DATASETS="gov_report" GPU_ID=1 \
bash llama/script/analysis/condition_block_gen/longbench/run_condition_block_triton_eps.sh 0.1

# 纯测速，不记录 budget
COLLECT_STATS=0 DATASETS="gov_report" GPU_ID=1 \
bash llama/script/analysis/condition_block_gen/longbench/run_condition_block_triton_eps.sh 0.1
```
