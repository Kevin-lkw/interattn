...
```bash
GPU_ID=7 bash script/analysis/condition_block_ppl/multisample/run_all.sh
```
...
CUDA_VISIBLE_DEVICES=7 conda run --no-capture-output -n nanogpt \
  python -m script.analysis.condition_block_ppl.multisample.run_attention_topk --device cuda:0

CUDA_VISIBLE_DEVICES=7 conda run --no-capture-output -n nanogpt \
  python -m script.analysis.condition_block_ppl.multisample.run_condition_block --device cuda:0

CUDA_VISIBLE_DEVICES=7 conda run --no-capture-output -n nanogpt \
  python -m script.analysis.condition_block_ppl.multisample.run_h2o --device cuda:0

CUDA_VISIBLE_DEVICES=7 conda run --no-capture-output -n nanogpt \
  python -m script.analysis.condition_block_ppl.multisample.run_streamllm --device cuda:0

CUDA_VISIBLE_DEVICES=7 conda run --no-capture-output -n nanogpt \
  python -m script.analysis.condition_block_ppl.multisample.run_quest --device cuda:0
```
