...
反推，约 4%–6% blocks 被展开。说明稀疏度已经较高，block 太小不是当前主要问题。更大的 block 可以减少 representative/selection 数量，但会让 range bound 更松、单次展开读取更多 token，并且需要新的 32/64-token fast kernel；它最多直接影响当前约 4.3% 的 condition+sparse CUDA 时间，应作为独立精度/速度实验，而不是首要端到端优化。
